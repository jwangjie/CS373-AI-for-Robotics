...
class robot:
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
          ...
          # (landmarks[i][0] - self.y) and (landmarks[i][1] - self.x), landmarks - x and landmarks - y these sequence matter
          if add_noise:
            bearing += random.gauss(0.0, self.bearing_noise) # creat a flag if we need to add noise 
          bearing %= 2.0 * pi
          Z.append(bearing)
        return Z 
...
def particle_filter(motions, measurements): # I know it's tempting, but don't change N!
    # --------
    #
    # Make particles
    # 
