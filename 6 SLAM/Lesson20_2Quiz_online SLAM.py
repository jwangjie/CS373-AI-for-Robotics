# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 11:58:22 2018
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    #
    # set the dimension of the filter
    # the reason for '2*' is because we are modeling x and y for the same data structures
    # difference 2 *(N + num_landmarks)
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            # [differences: get rid of 'n' of the value index]
            for b in range(2):
        ...
        ### differences to the non-optimal GraphSLAM in Lesson19_22Quiz_implementing SLAM.py
        ...
        exlist = [0, 1] + list(range(4, dim+2))
        Omega = Omega.expand(dim+2, dim+2, exlist, exlist)
        Xi    = Xi.expand(dim+2, 1, exlist, [0])
        
                
        # update the robot information matrix/vector based on the robot motion 
        # [differences: get rid of 'n' of the value index]
        ...
        # now factorizaiton will be applied to the previous pose [key difference to implement ONLINE SLAM]
...
result1 = online_slam(testdata1, 5, 3, 2.0, 2.0)
print ('From online SLAM:')
print_result(1,3,result1[0])
print ('From SLAM')
print_result(5,3,slam(testdata1, 5, 3, 2.0, 2.0))
solution_check(result1, answer_mu1, answer_omega1)
